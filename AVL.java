...
public class AVL<T extends Comparable<? super T>> {
    ...
    public void add(T data) {
        ...
        this.root = this.addNode(this.getRoot(), data);
    }

    private AVLNode<T> addNode(AVLNode<T> curr, T data) {
        if (curr == null) {
            this.size++;
            AVLNode<T> newNode = new AVLNode<T>(data);
            this.updateHeightAndBF(newNode);
            return newNode;
        }
        ...
        if (curr.getData().compareTo(data) > 0) {
            curr.setLeft(addNode(curr.getLeft(), data));
        } else {
            curr.setRight(addNode(curr.getRight(), data));
        }
    ...
    public T remove(T data) {
        ...
        AVLNode<T> dummy1 = new AVLNode<>(null);
        this.root = this.removeNode(this.root, dummy1, data);
        if (dummy1.getData() == null) {
            throw new NoSuchElementException();
        }
        return dummy1.getData();
    }

    private AVLNode<T> removeNode(AVLNode<T> curr, AVLNode<T> dummy1, T data) {
        if (curr != null) {
            // found
            if (curr.getData().compareTo(data) == 0) {
                // to 'return' data found
                dummy1.setData(curr.getData());
                this.size--;
                // 2 children:
                if (curr.getLeft() != null && curr.getRight() != null) {
                    AVLNode<T> dummy2 = new AVLNode<>(null);
                    curr.setRight(this.removeSuccessor(curr.getRight(), dummy2));
                ...
                curr.setLeft(removeNode(curr.getLeft(), dummy1, data));
            } else {
                curr.setRight(removeNode(curr.getRight(), dummy1, data));
            }
            updateHeightAndBF(curr);
            if (curr.getBalanceFactor() <= 2 || curr.getBalanceFactor() >= 2) {
                curr = this.balance(curr);
    ...
    private void updateHeightAndBF(AVLNode<T> currentNode) {
        ...
        currentNode.setHeight(Math.max(rightHeight, leftHeight) + 1);
